...
bool Go2HostDisplay::CheckSurface(u32 width, u32 height, DisplayPixelFormat format)
{
  if (width <= m_surface_width && height <= m_surface_height && format == m_surface_format)
    return true;

  static constexpr std::array<int, static_cast<u32>(DisplayPixelFormat::Count)> pf_mapping = {
    {DRM_FORMAT_RGBA8888, DRM_FORMAT_BGRA8888, DRM_FORMAT_RGB565, DRM_FORMAT_RGBA5551}};

  if (m_surface)
    go2_surface_destroy(m_surface);

  m_surface = go2_surface_create(m_display, width, height, pf_mapping[static_cast<u32>(format)]);
  if (!m_surface)
...
bool Go2HostDisplay::BeginSetDisplayPixels(DisplayPixelFormat format, u32 width, u32 height, void** out_buffer,
                                           u32* out_pitch)
{
  ...
  *out_pitch = static_cast<u32>(go2_surface_stride_get(m_surface));
  SetDisplayTexture(m_surface, m_surface_width, m_surface_height, 0, 0, width, height);
  return true;
}
...
bool Go2HostDisplay::Render()
{
  ...
  if (HasDisplayTexture())
  {
    ...
    go2_presenter_post(m_presenter, static_cast<go2_surface*>(m_display_texture_handle), m_display_texture_view_x,
                       m_display_texture_view_y, m_display_texture_view_width, m_display_texture_view_height, top, left,
                       height, width, GO2_ROTATION_DEGREES_270);
  }
