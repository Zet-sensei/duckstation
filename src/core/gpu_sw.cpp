...
void GPU_SW::CopyOut15Bit(u32 src_x, u32 src_y, u32 width, u32 height, u32 field, bool interlaced, bool interleaved)
{
  ...
  if (!interlaced)
  {
    if (!m_host_display->BeginSetDisplayPixels(HostDisplay::DisplayPixelFormat::RGB565, width, height,
  ...
  if ((src_x + width) <= VRAM_WIDTH && (src_y + height) <= VRAM_HEIGHT)
  {
    ...
    {
      CopyOutRow16ToRGB565(src_ptr, reinterpret_cast<u16*>(dst_ptr), width);
      src_ptr += src_step;
      dst_ptr += dst_stride;
    }
  }
  else
  {
    ...
    for (u32 row = 0; row < rows; row++)
    {
      ...
      for (u32 col = src_x; col < end_x; col++)
        *(dst_row_ptr++) = VRAM16ToRGB565(src_row_ptr[col % VRAM_WIDTH]);
  ...
  {
    m_host_display->SetDisplayPixels(HostDisplay::DisplayPixelFormat::RGB565, width, height,
