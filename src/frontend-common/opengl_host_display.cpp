...
{
  m_present_draw_framebuffer->sync_id = glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
  glFlush();

  std::unique_lock guard(m_present_lock);
  std::swap(m_present_draw_framebuffer, m_present_present_framebuffer);
  if (m_vsync)
  {
    if (!m_frame_presented.load())
    {
    ...
    {
      Log_WarningPrintf("Missed frame :(");
    }
  }
...
void OpenGLHostDisplay::PresentThread()
{
  ...
  while (!m_present_thread_stop.load())
  {
    {
      std::unique_lock guard(m_present_lock);
      PresentFramebuffer* fb = m_present_present_framebuffer;
      if (fb->sync_id)
      {
        glClientWaitSync(fb->sync_id, GL_SYNC_FLUSH_COMMANDS_BIT, GL_TIMEOUT_IGNORED);
        fb->sync_id = {};
      }
